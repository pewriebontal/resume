...
\newcommand{\entryHeaderLinked}[3]{\noindent\textbf{\href{#1?utm_source=resume&utm_medium=document&utm_campaign=experience}{#2}} \hfill \daterange{#3}}
\newcommand{\entrySubHeader}[1]{\textit{#1}}
\newcommand{\entrySubHeaderLinked}[2]{\textit{\href{#1?utm_source=resume&utm_medium=document&utm_campaign=experience}{#2}}}
...
\begin{center}
	...
	\begin{tabular}{@{}c@{}}
		...
		$if(mobile_raw)$ \quad|\quad \href{https://wa.me/$country_code$$mobile_raw$?utm_source=resume & utm_medium=document & utm_campaign=contact}{WhatsApp: +$country_code$ $mobile_display$}$else$ \quad|\quad Contact-on-request$endif$
		$if(location)$ \quad|\quad $location$$endif$                                                                                                                                                                                        \\[2pt]
		$if(portfolio)$\href{https://$portfolio$?utm_source=resume                                    & utm_medium=document & utm_campaign=portfolio}{$portfolio$}$endif$
		$if(linkedin)$ \quad|\quad \href{https://www.linkedin.com/in/$linkedin$?utm_source=resume     & utm_medium=document & utm_campaign=profile}{linkedin.com/in/$linkedin$}$endif$
		$if(github)$ \quad|\quad \href{https://github.com/$github$?utm_source=resume                  & utm_medium=document & utm_campaign=profile}{github.com/$github$}$endif$
	\end{tabular}
\end{center}
...
\noindent\textbf{\href{$experience.company_url$?utm_source=resume&utm_medium=document&utm_campaign=experience}{$experience.company$}}$if(experience.country)$, $experience.country$$else$$if(experience.country_code)$, $experience.country_code$$endif$$endif$ \hfill \daterange{$experience.period$}
$else$
...
\section*{Projects}
$for(projects)$
\noindent\textbf{$if(projects.url)$\href{$projects.url$?utm_source=resume&utm_medium=document&utm_campaign=projects}{$projects.name$\hspace{0.3em}\raisebox{-0.1em}{\small\faExternalLink}}$else$$projects.name$$endif$}$if(projects.tools)$ \quad|\quad \textit{\small $projects.tools$}$endif$ \hfill \textbf{$if(projects.institution_url)$\href{$projects.institution_url$?utm_source=resume&utm_medium=document&utm_campaign=projects}{$projects.institution$}$else$\textit{$projects.institution$}$endif$}
\begin{entryDetails}
...
$for(extracurricular)$
\noindent\textbf{$if(extracurricular.position_url)$\href{$extracurricular.position_url$?utm_source=resume&utm_medium=document&utm_campaign=extracurricular}{$extracurricular.position$}$else$$extracurricular.position$$endif$}, $extracurricular.organization$ \hfill \daterange{$extracurricular.period$}
\begin{entryDetails}
